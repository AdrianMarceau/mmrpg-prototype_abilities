    ...
    'ability_function' => function($objects){
        ...
        foreach ($backup_target_robots_active AS $key => $info){
            ...
            if ($temp_target_robot->robot_status != 'disabled'
                && $this_ability->ability_results['this_result'] != 'failure'
                && $temp_target_robot->counters['attack_mods'] > 0){
                $attack_breaks = $temp_target_robot->counters['attack_mods'];
                rpg_ability::ability_function_fixed_stat_break($temp_target_robot, 'attack', $attack_breaks, $this_ability, array(
                    'initiator_robot' => $this_robot
                    ));
            }
        }
