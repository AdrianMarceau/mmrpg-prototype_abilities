...
$functions = array(
    'ability_function' => function($objects){
        ...
        foreach ($extracted_core_shields AS $attachment_token => $attachment_info){
            ...
            if ($plural_shields){ $mid_text = $consumed_core_shields > 0 ? 'another' : 'the first'; }
            else { $mid_text = 'the'; }
            $core_shield_ability->set_frame_styles('transform: scale(0.5) translate(0, -100%); ');
            $core_shield_ability->target_options_update(array(
                'frame' => 'summon',
                'success' => array(3, 0, 10, 10, $this_robot->print_name().' consumes '.$mid_text.' '.$shield_name_span.'!')
                ));
            $this_robot->trigger_target($target_robot, $core_shield_ability, array('prevent_default_text' => true));
            $core_shield_ability->set_frame_styles('');

            // Restore this robot's energy with the consumed power if the robot has taken any damage
            if ($this_robot->robot_energy < $this_robot->robot_base_energy){
                $core_shield_ability->damage_options_update(array(
                    ...
                    'success' => array(0, 0, 0, -10, 'The '.$shield_name_span.' restored energy!'),
                    'failure' => array(0, 0, 0, -10, 'The '.$shield_name_span.' rolled past the target...')
                    ));
                $core_shield_ability->recovery_options_update(array(
                    ...
                    'kickback' => array(0, 0, 0),
                    'success' => array(0, 0, 0, -10, 'The '.$shield_name_span.' restored energy!'),
                    'failure' => array(0, 0, 0, -10, 'The '.$shield_name_span.' had no effect...')
                    ));
                $energy_recovery_amount = ceil($this_robot->robot_base_energy * ($core_shield_ability->ability_recovery / 100));
                $this_robot->trigger_recovery($this_robot, $core_shield_ability, $energy_recovery_amount);
            } else {
                $this_battle->queue_sound_effect('no-effect');
                $core_shield_ability->target_options_update(array('frame' => 'defend', 'success' => array(0, 0, 0, 10, '...but nothing happened.')));
                $this_robot->trigger_target($target_robot, $core_shield_ability, array('prevent_default_text' => true));
            }
            $consumed_core_shields++;
    ...
    'ability_function_onload' => function($objects){
        ...
        if ($this_robot->robot_token == 'ra-thor'){
            $this_ability->set_name($this_ability->ability_base_name.' R');
            $this_ability->set_recovery($this_ability->ability_base_recovery * 2);
        } else {
            $this_ability->reset_name();
            $this_ability->reset_recovery();
        }
